...
if __name__=='__main__':
	...
	for method in l_method:
		...
		elif method.lower() == 'eb':
			BOclass = BO_core.EB_mean_prior_BO
		elif method.lower() == 'ebu':
		...
		for r in tqdm.tqdm(range(args.repeat), desc='Runs'):
			...
			d_result[method] = y_rn


		if args.out is not None and len(args.out) > 0:
			np.savez(args.out.replace('.npz', '_{}.npz'.format(method)), x=x_rnd, y=y_rn)

	########################
	# visualize all
	########################
	l_label = ['base', 'DA', 'AL', 'EB', 'EBu']
	l_marker = ['o', '+', 's', '^', 'D']
	l_color = [
	(1., 0.851, 0.184),
	(0.553, 0.627, 0.796),
	...
	for i, method in enumerate(l_label):
		hndl = visualize.fill_plot(visualize.get_cummax(d_result[method]), label=l_label[i], marker=l_marker[i], color=l_color[i])
		l_hndl.append(hndl)
	plt.legend(handles=l_hndl, loc=0,)
