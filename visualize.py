...
def fill_plot(y_rn, **kwarg):
	if 'color' not in kwarg:
		kwarg['color'] = (.553,.674,.796)
	if 'marker' not in kwarg:
		kwarg['marker'] = 'o'

	N = y_rn.shape[1]
	plt.fill_between(np.arange(N)+1, np.min(y_rn, axis=0), np.max(y_rn, axis=0), alpha=.2, color=kwarg['color'])
	hndl, = plt.plot(np.arange(N)+1, np.mean(y_rn, axis=0), lw=2, markevery=10, **kwarg)
